...
// ----------------------------------------------------------------------------
use super::{super::{super::{Camera, Image, Material, Model, Texture},
                    LBFLight,
                    LBFMesh,
                    LBFObject,
                    LBF},
...
pub(crate) fn from_str(
    ...
    let materials = BTreeMap::<Uuid, Material>::from_lua(tbl.get(
        "materials",
    )?).unwrap_or_default();
    ...
    let armatures = BTreeMap::<Uuid, Armature<GLfloat>>::from_lua(tbl.get(
        "armatures",
    )?).unwrap_or_default();
